...
const createAddress = async (req, res) => {
    ...
    try {
        ...
        res.status(201).send({ status: true, message: 'Successfully address created' })
...
const updateAddress = async (req, res) => {
    try {
        ...
        const updatedAddress = await addressServiceInstance.updateAddress({ _id: addressId }, req.body, {
            new: true,
            runValidators: true
        })
        ...
        res.status(200).send({ status: true, message: "successfully address updated" })
    } catch (err) {
        res.status(500).json({ status: false, message: `error == ${err}` });
        console.log(err);
    }
...
const deleteAddress = async (req, res) => {
    try {
        const { id: addressId } = req.params
        const deletedAddress = await addressServiceInstance.deleteAddress({ _id: addressId })

        if (!deletedAddress) {
            return res.status(404).json({ status: 'address not delete' })
        }
        res.status(200).json({ status: true, message: "Successfully deleted address" })
...
const findAddressByUserId = async (req, res) => {
    try {
        ...
        const address = await addressServiceInstance.findAddressByUserId( { userId: userId })

        if (!address) {
            return res.status(404).json({ status: "address not found" })
        }
        res.status(200).json({ status: true, message: "Successfully find " , data: address})
